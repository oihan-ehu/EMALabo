...
barplot3d(rows=2, cols = 2, z=probabilities, xlabels = c("A","B"), ylabels= c("A", "B"),
          topcolors="grey")
rglwidget(); close3d()
```
...
𝑦
2
 funtzioa irudikatu nahi dugula, non 𝑥,𝑦∈IR
𝑥
,
𝑦
∈
I
R
. Azken finean, gainazal bat da irudikatu behar duguna. Horrela, lehenik eta behin, zein 𝑥
𝑥
 eta 𝑦
𝑦
 balio multzotarako irudikatu nahi dugun zehaztu behar dugu. Adibidez:
...
Matrize bat da, z[i][j] f(x[i], y[j]) delarik.
...
```{r, fig.align='center'}
persp(x, y, z, phi=30, theta=70, 
      main="f GAINAZALA", col="gray", border=NA, ltheta = -20, lphi = 30, shade=0.7)
```
...
```{r, fig.width=4, fig.align='center'}
image(x, y, z, col = gray.colors(40))
contour(x, y, z, add=TRUE)
```
...
barplot3d(rows=4, cols=4, z=z, ylabels = x, xlabels = y)
rglwidget(); close3d()
...
persp3d(x, y, z, shade=0.4, ltheta=34, lphi=-10, col="gray")
rglwidget(); close3d()
...
f <- function(x, Sigma=diag(length(x)), m=rep(0, length(x))){
  k <- length(x)
  det_Sigma <- det(Sigma)
  inv_Sigma <- solve(Sigma)
  coef <- 1 / ((2 * pi)^(k / 2) * sqrt(det_Sigma))
  exp_term <- exp(-0.5 * t(x) %*% inv_Sigma %*% x)
  return(as.numeric(coef * exp_term))
}
...
var1 <- runif(1, 0.3, 3)
var2 <- runif(1, 0.3, 3)
cov <- runif(1, -sqrt(var1*var2), sqrt(var1*var2))
sigma <- matrix(c(var1, cov, cov, var2), 2)

y <- x <- seq(-10, 10, length.out=40)
xy <- expand.grid(x, y)
density <- apply(xy, 1, function(row)f(as.numeric(row), Sigma = sigma))
density2 <- apply(xy, 1, function(row)dmvnorm(as.numeric(row), sigma = sigma))

# f
persp3d(x, y, matrix(density, 40),
      main="Dentstate funtzioa f",
      col="gray",zlab = "Denstitatea",
      border=NA, ltheta = -20, lphi = 30, shade=0.7)
rglwidget(); close3d()

# dmvnorm
persp3d(x, y, matrix(density2, 40),
      main="Dentstate funtzioa dmvnorm",
      col="gray",zlab = "Denstitatea",
      border=NA, ltheta = -20, lphi = 30, shade=0.7)
rglwidget(); close3d()

# diferentzia
persp3d(x, y, matrix(density-density2, 40),
      main="Dentstate funtzioen diferentzia",
      col="gray",zlab = "Denstitatea",
      border=NA, ltheta = -20, lphi = 30, shade=0.7)
rglwidget(); close3d()
...
```{r, fig.align='center'}
dirichlet <- function(x, alpha=rep(1, length(x))) {
  if (any(x <= 0) || abs(sum(x) - 1) > 1e-6) return(0)
  
  gamma_sum <- gamma(sum(alpha))
  gamma_prod <- prod(gamma(alpha))
  coef <- gamma_sum / gamma_prod # Normalizazioa
  
  prod_term <- prod(x^(alpha - 1))
  return(coef * prod_term)
}
```
...
alpha <- c(0.2, 0.6)
xy <- expand.grid(x, y)
density <- apply(xy, 1, function(row)dirichlet(as.numeric(row), alpha = alpha))
density <- matrix(density, n)

plot(x, colSums(density), t="l", xlab = "Dentsitatea")
```
...
alpha <- c(0.9, 0.8, 2)
xy <- expand.grid(x, y)
density <- apply(xy, 1, function(row)dirichlet(as.numeric(c(row, 1-sum(row))), alpha = alpha))
density <- matrix(density, n)
...
image(x, y, density, col = gray.colors(1000))
contour(x, y, density, add=TRUE)
```
