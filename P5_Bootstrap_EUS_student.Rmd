...
```{r}
par(mfrow=c(3, 3))
for (n in c(10, 100, 1000, 10000, 100000, 500000)){
  hist(rnorm(100000, 5, 2/sqrt(n)), main=paste("Estimatzailearen banaketa\nn =", n),
      xlab="Estimazio-balioak", xlim=c(2.5, 7.5))
  abline(v=5, col="red")
}
...
Implementatuko dugun lehenengo funtzioak estimatzailearen alborapena hurbiltzen du. Alborapena $b(\hat{\theta})=E(\hat{\theta})-\theta$ da. Hau hurbiltzeko bootstrap metodoa eta plug-in printzipioa aplikatuko ditugu, itxaropena bootstrap estimazioen batezbestekoarengatik ordezkatuz ($E(\hat{\theta})=\bar{\hat{\theta}^*}$) eta benetako parametroaren balioa estimazioa erabiliz ordezkatuz ($\theta \sim \hat{\theta}$). 

$$b(\hat\theta) \simeq b^*(\hat\theta)=\bar{\hat\theta^*} - \bar\theta_\mathbf{x}$$
...
$$SE^*(\hat\theta)=\sqrt{\frac{1}{B-1}\sum_{i=1}^B(\hat\theta_i^* -\bar{\hat\theta^*})^2}$$
...
getCI <- function (x, alpha = 0.05)
{
  ...
  t <- qt(1 - alpha/2, df)  # t-critical value
...
{
    n <- length(x)
    boot.laginak  <- bootstrapLaginakLortu(x, B, n)
    boot.estimazioak <- apply(boot.laginak, 1, FUN, ...)
    ci <- quantile(boot.estimazioak, c((alpha/2), (1-alpha/2)))
    names(ci) <- paste("%", c((alpha/2)*100, (1-alpha/2)*100), sep="")
  return(ci)
}
...
$$t=\frac{\hat\theta-\theta}{SE(\hat\theta)}\xrightarrow{Bootstrap}t^*=\frac{\hat\theta^*-\hat\theta}{SE(\hat\theta^*)}$$


```{r, eval=TRUE}
getCIBootTmean <- function(x, alpha=0.05, B=1000, closed=TRUE)
{
  estimazioa <- mean(x)
  n <- length(x)
  boot.laginak  <- bootstrapLaginakLortu(x, B, n)
  boot.estimazioak <- apply(boot.laginak, 1, mean)
  se <- sd(x)/length(x)
  boot.t <- approxDistributionTBootmean(estimazioa, boot.laginak, boot.estimazioak)
  ci_inf <- estimazioa - quantile(boot.t, 1-alpha/2)*se
  ci_sup <- estimazioa - quantile(boot.t, alpha/2)*se
  ci <- c(ci_inf, ci_sup)
  names(ci) <- paste("%", c((alpha/2)*100, (1-alpha/2)*100), sep="")
  return(ci)
}

approxDistributionTBootmean <-function(estimation, boot.samples, boot.estimations){
  n <- ncol(boot.samples)
  B <- nrow(boot.samples)
  boot.se <- apply(boot.samples, 1, sd)/sqrt(n)
  t.balioak <- (boot.estimations - estimation)/boot.se
}

getCIBootTmean(lagina)
```

*Errore estandarrerako formula itxia ez dugunean* bootstrap lagin bakoitzerako bootstrap lagin gehiago atera behar ditugu:

```{r, eval=TRUE}
getCIBootT <- function(x, alpha=0.05, B=1000, B2=100, estim_fun=mean, ...)
{
  estimazioa <- estim_fun(x, ...)
  n <- length(x)
  boot.laginak  <- bootstrapLaginakLortu(x, B, n)
  boot.estimazioak <- apply(boot.laginak, 1, estim_fun, ...)
  se <- sd(boot.estimazioak)
  boot.t <- approxDistributionTBoot(estimazioa, boot.laginak, boot.estimazioak, B2, estim_fun, ...)
  ci_inf <- estimazioa - quantile(boot.t, 1-alpha/2)*se
  ci_sup <- estimazioa - quantile(boot.t, alpha/2)*se
  ci <- c(ci_inf, ci_sup)
  names(ci) <- paste("%", c((alpha/2)*100, (1-alpha/2)*100), sep="")
...
approxDistributionTBoot <-function(estimation, boot.samples, boot.estimations, B2=100, estim_fun=mean, ...){
  n <- ncol(boot.samples)
  B <- nrow(boot.samples)
  ...
  for (i in 1:B){
    # Bootstrap lagin bakoitzeko azpi-bootstrap bat egin
    sample_i <- boot.samples[i,]
    
    # Bigarren mailako bootsrap sortu
    boot2.laginak <- bootstrapLaginakLortu(sample_i, B2, n)
    boot2.estimazioak <- apply(boot2.laginak, 1, estim_fun, ...)
    
    # Errore estandarraren urbilpena
    boot.se <- sd(boot2.estimazioak)
    
    # t.balioetara gehitu
    t.values[i] <- (boot.estimations[i] - estimation)/boot.se
  }
...
*Erantzuna:* 1. lagina banaketa normalaren itxura dauka eta 2. lagina banaketa exponentzialarena

  
+ Batezbestekoaren bootstrap banaketa irudikatu eta kalkulatu alborapena eta errore estandarra bi kasuetan.

```{r}
boot.laginak1 <- bootstrapLaginakLortu(lagina1, 1000, length(lagina1))
boot.estimazioak1 <- apply(boot.laginak1, 1, mean)
hist(boot.estimazioak1)
se1 <- estimatuSE(boot.estimazioak1)
se1
alb1 <- estimatuAlborapena(boot.estimazioak1, mean(lagina1))
alb1

boot.laginak2 <- bootstrapLaginakLortu(lagina2, 1000, length(lagina2))
boot.estimazioak2 <- apply(boot.laginak2, 1, mean)
hist(boot.estimazioak2)
se2 <- estimatuSE(boot.estimazioak2)
se2
alb2 <- estimatuAlborapena(boot.estimazioak2, mean(lagina2))
alb2
```
...
```{r}
boot.laginak1 <- bootstrapLaginakLortu(lagina1, 1000, length(lagina1))
boot.estimazioak1 <- apply(boot.laginak1, 1, var)
hist(boot.estimazioak1)
se1 <- estimatuSE(boot.estimazioak1)
se1
alb1 <- estimatuAlborapena(boot.estimazioak1, mean(lagina1))
...
boot.laginak2 <- bootstrapLaginakLortu(lagina2, 1000, length(lagina2))
boot.estimazioak2 <- apply(boot.laginak2, 1, var)
hist(boot.estimazioak2)
se2 <- estimatuSE(boot.estimazioak2)
se2
alb2 <- estimatuAlborapena(boot.estimazioak2, mean(lagina2))
...
```{r}
getCIBootQuantile(lagina1, FUN = mean)

getCIBootQuantile(lagina2, FUN = mean)
```
  
  
  + Bootstrap t-konfiantza tarteak.
  
*batazbestekoaren SE-rako formula itxia dago*
  
```{r}
getCIBootTmean(lagina1)

getCIBootTmean(lagina2)
```
...
```{r}
getCIBootQuantile(lagina1, FUN=var)

getCIBootQuantile(lagina2, FUN=var)
```
  
  + Bootstrap t-konfiantza tarteak (OHARRA: Kasu honetan estimatzailearen benetako SE ez da ezaguna, ikus 53. gardenkia).

```{r}
getCIBootT(lagina1, estim_fun = var)

getCIBootT(lagina2, estim_fun = var)
```
...
n = 100
N = 1000
laginak <- matrix(rnorm(n*N, 20, 5), ncol = n, nrow = N)

testCI <- function(lagina, FUN1=getCI, ...){
    apply(laginak, 1, function(x){
    ci <- FUN1(x, ...)
    return (20>ci[1]&&20<ci[2])
  })
}

barruanCI <- testCI(laginak, FUN1=getCI)

barruanCIQuantile <- testCI(laginak, FUN1=getCIBootQuantile)
#barruanCIT <- testCI(laginak, FUN1 = getCIBootT)

sum(barruanCI)/N
sum(barruanCIQuantile)/N
#sum(barruanCIT)/N
```
