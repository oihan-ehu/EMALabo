...
```{r}
x1.teoriko <- c(1, 0, 0, 0) %*% trantsizio.matrizea %*% matrix(c(1, 1, 1, 0, 0, 0, 0, 0, 0), nrow=3, ncol=3)
names(x1.teoriko) <- egoerak
x2.teoriko <- x1.teoriko %*% trantsizio.matrizea
x3.teoriko <- x2.teoriko %*% trantsizio.matrizea

p.teoriko <- cbind(x1.teoriko, x2.teoriko, x3.teoriko, deparse.level = 0)
for (i in 4:100)
  p.teoriko <- cbind(p.teoriko, p.teoriko %*% )
```
