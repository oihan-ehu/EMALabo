...
```{r}
EX.analitiko <- alfa.aposteriori/(alfa.aposteriori + beta.aposteriori)
EX.analitiko
```
...
```{r}
EX.mcmc <- mean(aposteriori.lagina[5000:50000])
EX.mcmc
```
...
```{r}
TB.analitiko <- qbeta(c(0.025, 0.975), alfa.aposteriori, beta.aposteriori)
names(TB.analitiko) <- paste("%", c(2.5, 97.5), sep = "")
TB.analitiko
```
...
```{r}
TB.mcmc <- quantile(aposteriori.lagina[5000:50000], c(0.025, 0.975))
names(TB.mcmc) <- paste("%", c(2.5, 97.5), sep = "")
TB.mcmc
```
...
Lehenengo MCMC-rako funtzio orokor bat sortuko dut:

```{r}
mcmc <- function(lagin.kopurua, # Zenbat lagin sortu
                 df, # Lagindu nahi den dentsitate funtzioaren proportzionala
                 dg, # g-ren dentsitate funtzioa
                 rg, # g lagintzeko funtzioa
                 x1 # lehenengo balioa
                 ){
  lagina <- x1
  
  for (i in 1:lagin.kopurua) {
    balio.posible <- rg(lagina[i])
    
    ratio.zati <- df(lagina[i])*dg(balio.posible, lagina[i])
    if (ratio.zati != 0){
      ratio <- df(balio.posible)*dg(lagina[i], balio.posible)/ratio.zati
    } else {
      ratio <- 1
    }
    ...
    if (ratio > runif(1)){
      ...
      balioa.gehitzeko <- lagina[i]
    }
    
    lagina <- c(lagina, balioa.gehitzeko)
  }
...
Orain definituko ditut \texttt{df, dg} eta \texttt{rg}

```{r}

df <- function(x) {
  (x^binomial.true * (1-x) ^ binomial.false) *
    dbeta(x, alfa.apriori, beta.apriori)
}

for (w in c(0.0001, 0.1, 1, 100, 1000)){
  w <- 0.01
  ...
  lagina <- mcmc(50000, df, dg, rg, rbeta(1, alfa.apriori, beta.apriori))
  
  # Irudikatu
  hist(lagina[45001:50000], main=paste("Lagindutako balioen histograma azken 1000 pausoetan w =", w))
  plot(49001:50000, lagina[49001:50000], type="l", col="darkgreen", 
       xlab="Pausoa", ylab="Theta", main=paste("Lagindutako balioak azken 1000 pausoetan w =", w))
}
```
...
*Erantzuna:* \texttt{w} handiegia bada 0 dentsitatea duen puntu askotan eroriko da algoritmoa, ondorioz kate luzeak egongo dira non balioak ez diren aldatzen. \texttt{w} txikiegia bada, ordea, nahiz eta aldaketa gehiago gertatu, aldaketa horiek oso txikiak izango dira.
